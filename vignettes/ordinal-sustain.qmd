...
trax_gp34_v1 |> graph_CGG_repeats(rows = vars(Gender))

```

```{r}
#| label: subset-data
n_missing_CGG = trax_gp34_v1$CGG |> is.na() |> sum()
n_above_200 = sum(trax_gp34_v1$CGG >= 200, na.rm = TRUE)
gp34_v1_usable = trax_gp34_v1 |> filter(CGG < 200)
...
We also fit the model on the full dataset (not stratified by sex or CGG) for 2-`r N_S_max` latent subgroups, each with their own ordering (@fig-pvd_optimal). We determined the optimal number of latent subgroups for this dataset using the CVIC criterion (@sec-cvic). 
