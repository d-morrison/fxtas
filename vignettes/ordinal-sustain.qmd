...
To test for differences in event sequence between strata such as sex and CGG repeat level, we implemented a permutation test, in which we created 1000 permuted datasets in which the strata variables were randomly shuffled. We computed the mean log-likelihood of the data for each permuted dataset (averaging across MCMC samples and summing across strata) and compared the distribution of permuted mean log-likelihoods to the observed log-likelihood calculated from the original (unpermuted) dataset. @fig-permute-llik-sex compares the observed log-likelihood for the sex-stratified model to the distribution of mean log-likelihoods when sex is permuted.
