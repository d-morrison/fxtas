...
`r nrow(gp34_v1_controls)` controls) were included in the event sequence analysis. @tbl-demographics provides demographic information about the study participants. 
...
In order to the Ordinal SuStaIn algorithm, we must specify how many latent subtypes to model. There are several metrics for determining the optimal number of subtypes for a given data set.
...
We performed `r N_CV_folds`-fold cross-validation on the unstratified GP+GP4 data, and calculated the CVIC for 1-`r N_S_max` latent subtypes.
