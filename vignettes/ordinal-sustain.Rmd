...
nlevs = sapply(X = df[,biomarkers], F = function(x) length(levels(x)))
...
score_vals = matrix(
  ...
  dimnames = list(biomarkers, ModelScores[-1]))
...
output_folder = "output.FXTAS1" |> fs::dir_create()
print(output_folder)
```
...
samples_likelihood, fig0] = sustain_input.run_sustain_algorithm(plot=True)
