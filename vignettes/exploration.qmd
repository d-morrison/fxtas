...
```{r}
footnote <- paste0(
  "Reason for missing age of onset in individuals with ",
  "'Some tremor recorded'"
)

table1(
  NA.label = NA.label1,
  stratified_formula("Tremor: Age of onset: missingness reasons", column_var),
  data = visit1 |> 
    filter(`Any tremor (excluding Head Tremor)` == "Some tremors recorded"),
  footnote = footnote)
...
table1(
  NA.label = NA.label1,
  stratified_formula("Head Tremor: Age of onset: missingness reasons", column_var),
  data = visit1 |> filter(Study == "GP4" & `Head tremor` == "Yes"))
