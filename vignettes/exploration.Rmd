...
library(pander)
library(table1)
NA.label = "Field empty in RedCap"
...
There is substantial missingness in APOE in the first visit records:
...
apoe.n = gp34 |> 
  group_by(`FXS ID`) |> 
  summarize(
    first_missing = is.na(ApoE[1]),
    any_missing = any(is.na(ApoE)),
    any_nonmissing = any(!is.na(ApoE)),
    n_vals = n_distinct(ApoE, na.rm = TRUE))
...
Is it reasonable to use the next non-missing ApoE result to impute the ApoE results for the earlier visit? Note that in some cases, the ApoE value changes between visits:
