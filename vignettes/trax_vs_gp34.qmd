...
biomarker_levels = 
  lapply(gp34_df[,biomarker_varnames], F = levels)

trax_gp34_df = 
  males_gp34_trax_v1 |> 
  filter(!is.na(`FX*`))

biomarker_levels_trax_gp34 = 
  lapply(trax_gp34_df[,biomarker_varnames], F = levels)
...
{
  if(setequal(gp34_df[[i]], trax_gp34_df[[i]]))
  { 
    diffs[[i]] = "no diffs"
  } else
  {
    diffs[[i]] = list(
      gp34 = setdiff(gp34_df[[i]], trax_gp34_df[[i]]),
      trax = setdiff(trax_gp34_df[[i]], gp34_df[[i]]))
  }
}
...
score_vals_trax_gp34 = model_dists(
  df = trax_gp34_df,
...
fig_trax_only = extract_figs_from_pickle(
  ...
  output_folder = "output/trax_only_visit1") |> 
  print()
