...
data("gp34")
scid_dat <- gp34 |>
  dplyr::select(`FXS ID`, all_of(scid_vars))

# convert to long format
scid_lifetime_long <- scid_dat |>
  dplyr::select(`FXS ID`, all_of(lifetime_vars)) |>
  pivot_longer(
...
scid_current_long <- scid_dat |>
  dplyr::select(`FXS ID`, all_of(current_vars)) |>
  pivot_longer(
...
scid_long <- merge(scid_current_long, scid_lifetime_long, 
                   by = c("FXS ID", "Variable"), sort = FALSE) |>
  mutate(
    Variable = factor(
...
# test scid domains
scid_domains <- create_scid_domain(gp34 |> get_visit1())
```
