...
ids <- unique(c(gp3_ids, gp4_ids, trax_ids))

study_data <- data.frame(
  "ID" = ids,
  "GP3" = ifelse(ids %in% gp3_ids, "GP3", NA_character_),
  "GP4" = ifelse(ids %in% gp4_ids, "GP4", NA_character_),
  "TRAX" = ifelse(ids %in% trax_ids, "TRAX", NA_character_)
) |>
  unite(
...
```{r }
table1(~ Studies, data = study_data)
```
