...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = trax_visit1 |> dplyr::filter(Study == "GP4"),
  footnote = footnote)
...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = trax_visit1 |> dplyr::filter(Study == "GP4" & `Head tremor` == "Yes"),
  footnote = footnote)
...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = trax_visit1 |> dplyr::filter(Ataxia == "Yes"))
```
...
table1(na.is.category = FALSE, 
  ...
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  footnote = footnote,
  data = trax_visit1 |> dplyr::filter(Ataxia == "No" | is.na(Ataxia))
)
```
...
) # |> sapply(FUN = function(x) grep(value =  TRUE, fixed = TRUE, pattern = x, x = names(trax_visit1))) |> as.vector()
...
cantab_vars = c(
  ...
  "RTI Five-choice movement time*") 
# |> 
#   sapply(FUN = function(x) grep(value =  TRUE, fixed = TRUE, pattern = x, x = names(trax_visit1))) |> 
# as.vector()
