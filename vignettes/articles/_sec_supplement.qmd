...
prob_correct |> attr("data") |>
  mutate(
    "# at baseline (%)" = glue::glue("{`# at baseline`}/{`# obs`}, ({round(`% at baseline`* 100, 1)})")
  ) |>
  select(Biomarker = biomarker, `# at baseline (%)`, "Pr(correct)" = prob_correct) |>
  pander::pander()
```
