...
compact_fig_caption <- paste0(
  "The different colors correspond to the different clinically elevated levels. ",
  "Red corresponds to the first clinically elevated level, blue to the second level, ",
  "purple to the third level, green to the fourth level, ",
  "and magenta to the fifth level. ",
...
Each clinically elevated ordinal level (above the reference level) constitutes an outcome event 
...
In order to simplify the analysis and produce more robust results, we created composite variables for `"any autoimmune disorder"` and five domains of the Structured Clinical Interview for DSM Disorders (SCID) (details in Supplementary Materials, @sec-composites).
...
Each event occurs at some point in continuing time 
relative to other events in sequence.
The exact onset times of the events 
...
We found significant evidence of sex differences in proportions of clinically elevated (non-reference) biomarker levels at baseline visit for `r report_sex_differences(biomarkers_table)`.
...
The significant differences for these subgroups 
...
The missing data contributes to the 
large amount of uncertainty in the results.
...
#| fig-cap: !expr glue::glue("**Event sequence models stratified by sex. \\(a\\)** Uncertainty in estimated event sequences models. **(b)** Positional differences in estimated event sequence between sexes. {compact_fig_caption}")
#| layout-ncol: 1
#| fig-subcap: 
#|    - " "
#|    - " "
...
#| fig-height: !expr pvd_height
#| fig-width: !expr pvd_width
#| fig-cap: !expr glue::glue("**Event sequences stratified by sex.** **\\(a\\)** Estimated event sequences, stratified by sex. **(b)** Differences in event sequences between sexes, for CGG repeats <100. {compact_fig_caption}")
#| layout-ncol: 1
#| fig-subcap: 
#|    - ""
#|    - ""
