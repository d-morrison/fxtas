...
Here, we apply the Ordinal **Su**btype and **St**age **In**ference ("SuStaIn") algorithm [@young2021ordinal] 
to find event sequences for FXTAS patients.
Ordinal SuStaIn is a data-driven analytic approach 
which analyzes cross-sectional biomarker data 
to simultaneously 
...
Ordinal SuStaIn has been used in other diseases (Alzheimers, [other diseases](https://scholar.google.com/scholar?cites=995536064967683113&as_sdt=5,39&sciodt=0,39&hl=en>)), but not previously for Fragile X Syndrome. 
We now apply it to a cross-sectional data set of Fragile X premutation carriers,
to study the sequential ordering of disease progression events in Fragile X premutation.
...
We applied an event-based disease progression model to our data using the 
...
event-based disease progression modelling [@fonteijn2012event] 
...
which are heatmaps with biomarker events on the y-axis 
and sequence positions of events, 
with each event occurring at some point in continuing time 
relative to other events in sequence, on the x-axis.
It should be pointed out that the exact onset times of the events 
...
That is, brighter colors indicate a more probable sequence position, 
and paler colors indicate a less probable position. 
...
**clinical folks: what is our policy for data availability? Here's one option:**
...
When we fit models by sex and CGG repeats level, 
...
and @tbl-sg_demos shows the demographics of the patients clustered in each subtype. 
...
we categorized some biomarkers that were originally measured as numbers.
In doing so, we sacrificed some granular information. 
There are other variations of the SuStaIn algorithm, 
such as z-score SuStaIn (@young2018uncovering) 
which are designed to be used with continuous biomarker measurements,
and it might be feasible to combine Ordinal SuStaIn and z-score SuStaIn
to fit an event-based model with both continuous and ordinal outcomes. 

### Comparison with longitudinal data

Our data come from the GP and Trax studies, which are longitudinal cohort studies
with infrequent followup visits.
These studies do not collect precise information about timing of symptom onsets, 
so it is not feasible to use the longitudinal data to confirm the results from the Ordinal SuStaIn analysis.
Additional research including longitudinal data collecting precise ages of onset 
would be needed to to verify our findings.
...
#| tbl-cap: "Descriptive statistics of patient characteristics by CGG repeats level. P-values represent tests for differences in distributions of characteristics between sexes, ignoring CGG repeat level."
...
```{r}
#| tbl-cap: !expr glue::glue("Demographics of latent subtype clusters identified by Ordinal SuStaIn ({n_s_selected} subtypes)")
#| label: tbl-sg_demos
#| results: asis
...
#| fig-height: !expr pvd_height
#| fig-width: !expr pvd_width
#| fig-cap: !expr glue::glue("\\(a\\) Estimated event sequences, stratified by CGG repeats (<100 vs 100+) and (b) differences in event sequences between CGG repeat sizes. {compact_fig_caption}")
#| layout-ncol: 1
#| fig-subcap: 
#|    - ""
#|    - ""
...
#| fig-height: !expr pvd_height
#| fig-width: !expr pvd_width
#| fig-cap: !expr glue::glue("\\(a\\) Estimated event sequences, stratified by sex and (b) differences in event sequences between sexes, for CGG repeats <100. {compact_fig_caption}")
#| layout-ncol: 1
#| fig-subcap: 
#|    - ""
#|    - ""
...
#| fig-height: !expr pvd_height
#| fig-width: !expr pvd_width
#| fig-cap: !expr glue::glue("\\(a\\) Estimated event sequences, stratified by sex and (b-d) differences in event sequences between sexes, for {n_s_selected} latent subtypes. {compact_fig_caption}") 
#| fig-cap-location: top
#| layout-ncol: 1
#| layout-nrow: 4
#| fig-subcap: 
