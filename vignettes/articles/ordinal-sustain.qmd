...
were missing completely at random (MCAR) (@rubin1976inference).
...
Our data included `r nrow(v1_usable_cases)` Fragile X premutation carriers and `r nrow(gp34_v1_controls)` controls;
`nrow(males)` males and `nrow(females)` females;
FXTAS stage, biomarkers etc.
