...
The fragile X-associated tremor/ataxia syndrome (FXTAS) is a  neurodegenerative disorder caused by the premutation (55 to 200 CGG repeats) in the FMR1 gene.
A currently open question is: In what sequential order do FXTAS symptoms typically appear, how does that sequence vary between participants and between males and females?
Here, we applied the Ordinal **Su**btype and **St**age **In**ference ("SuStaIn") algorithm to identify the sequential events of neurophenotypic symptoms presented in observed data collected in FXTAS patients.
We found substantial differences in order of events depending on sex and CGG repeat number.

# Introduction

The fragile X-associated tremor/ataxia syndrome (FXTAS) is a neurodegenerative disorder caused by the premutation (55 to 200 CGG repeats) in the FMR1 gene [@hagerman2001intention]. 
...
**CLINICAL TEAM: please complete this section and fill in detailed descriptions of the GP and Trax cohorts. Cite refs as needed**.
...
(2) CGG repeats (<100 vs ≥100) (@fig-pvd-by-cgg);
...
we assigned probabilities of `r probs` 
to Ataxia Severity levels `r and::and(0:4)`, respectively, 
...
However, correlations between repeated observations of the same individuals 
are not accounted for by the Ordinal SuStaIn model, 
which assumes that all observations are statistically independent, 
so including repeated measurements could lead to exaggerated estimates of precision.

### Visualizing modeling results

We visualized the results of Ordinal SuStaIn analysis using "positional variance diagrams" (PVDs), 
which are heatmaps 
with biomarker events on the y-axis 
and sequence positions of events, 
with each event occurring at some point in continuing time relative to other events in sequence, on the x-axis.
It should be pointed out that the exact onset times of the events cannot be determined by the method we used for this analysis.
...
That is, brighter colors indicate a more probable sequence position, and paler colors indicate a less probable position. 
...
sustain_output_cgg100minus = run_and_save_OSA(
  ...
  use_parallel_startpoints = use_parallel_startpoints,
  seed = 1,
  plot = plot_python,   fig_size = fig_size)
...
The data used in this analysis may be provided upon request from the 
...
and `r and::and(stages)` carriers at FXTAS stages `r names(stages) |> and::and()`, respectively. We found significant evidence of sex differences in proportions of events at baseline visit for `r report_sex_differences(biomarkers_table)`.
...
between males and females (p = `r pval_sex_gp34_v1`) (@fig-first-only-2). 
It appears that 
female premutation carriers developed threshold-level Mood Disorder symptoms prior to FXTAS Stage 1 diagnosis,
whereas male carriers developed these symptoms between FXTAS Stages 3 and 4.
Cognitive declines, as measured by SWM and PAL also occurred at earlier FXTAS stages for females.
Ataxia occurred prior to Stage 1 for females and after Stage 2 for males.
However, Ataxia severity 1 occurred earlier for males (between Stages 2-3) than for females (between Stages 3-4).
...
some differences were noted (@fig-pvd-by-cgg-2).
SWM between errors occurred prior to Stage 1 in participants with CGG repeats <100, 
where it occurred between stage 4 and 5 in those with CGG repeats ≥ 100.
Postural, resting, head, and intermittent tremor occurred earlier in those with CGG repeats <100 compared to those with CGG repeats ≥ 100.  
Several psychiatric disorders (as measured by SCID) occurred later in the event sequence in participants with CGG repeats < 100 compared to those with CGG repeats ≥ 100. 
After stratifying by sex, it appeared that both males and females 

These differences appeared to be more pronounced among males than females 
...
@fig-pvd-by-gender-cgg_under100 shows the estimated event sequences for CGG < 100 stratified by sex and compares the estiamted event sequences between males and females.
...
Several psychiatric disorders (as measured by SCID) occurred prior to FXTAS Stage 1 among females with CGG < 100, 
...
It should noted that the significant differences for these subgroups 
needs to be declared with caution due to the small sample sizes.
...
We found significant evidence of a difference between CGG < 100 
and CGG ≥ 100 among males (p = `r pval_male_cgg_gp34_v1`). 
@fig-pvd-by-cgg-male shows the corresponding stratified models and  
compares the estimates of the sequences for these two subgroups.
Males with CGG ≥ 100 experienced MRI biomarkers at earlier FXTAS stages 
than males with CGG < 100, 
whereas postural tremor, resting tremor, and intermittent tremor 
all tended to occur at earlier FXTAS stages for males with CGG < 100.
...
We did not find statistically significant evidence of a difference between CGG < 100 and CGG ≥ 100 among females (p = `r pval_female_cgg_gp34_v1`). 
@fig-pvd-by-cgg-female shows the estimated stratified models and compares the estimates. 
Here again, sample size was an issue, especially since the available data included only `r n_over100_Female` females with CGG ≥ 100.
...
Based on the CVIC criterion ([Supplementary section @sec-cvic]), 
`r n_s_min_CVIC` subtypes were suggested to be the optimal number for the full, unstratified dataset (@fig-cvic). The out-of-fold log-likelihood (Supplementary Material @eq-def-oof-llik) showed substantial fold-to-fold variation.
...
From these results, it did not appear that there is much benefit to including
...
@fig-pvd_chosen_subtypes shows positional variance diagrams (PVDs) for each of the subtype clusters detected under this assumption, and @tbl-sg_demos shows the demographics of the patients clustered in each subtype. 
...
whereas males dominated in subtype 3 (29% versus 12%). Subtypes 1 and 4 were equally prevalent among men and women.
...
#| tbl-cap: "Descriptive statistics of patient characteristics by CGG repeats level"
...
#| fig-height: !expr pvd_height
#| fig-width: !expr pvd_width
#| fig-cap: !expr glue::glue("\\(a\\) Estimated event sequences, stratified by CGG repeats (<100 vs 100+) and (b) positional differences in estimated event sequence between CGG repeats. {compact_fig_caption}")
#| layout-ncol: 1
#| fig-subcap: 
#|    - ""
#|    - ""
...
#| fig-height: !expr pvd_height
#| fig-width: !expr pvd_width
#| fig-cap: !expr glue::glue("\\(a\\) Estimated event sequences, stratified by CGG repeats (<100 vs 100+) and (b) positional differences in estimated event sequence between CGG repeats, males only. {compact_fig_caption}")
#| layout-ncol: 1
#| fig-subcap: 
#|    - ""
#|    - ""
...
#| fig-height: !expr pvd_height
#| fig-width: !expr pvd_width
#| fig-cap: !expr glue::glue("\\(a\\) Estimated event sequences, stratified by CGG repeats (<100 vs 100+) and (b) positional differences in estimated event sequence between CGG repeats; females only. {compact_fig_caption}")
#| layout-ncol: 1
#| fig-subcap: 
#|    - ""
#|    - ""
...
#| fig-height: !expr pvd_height
#| fig-width: !expr pvd_width
#| fig-cap: !expr glue::glue("\\(a\\) Estimated event sequences, stratified by sex and (b) positional differences in estimated event sequence between sexes, for CGG repeats <100. {compact_fig_caption}")
#| layout-ncol: 1
#| fig-subcap: 
#|    - ""
#|    - ""
...
```{r}
#| label: fig-pvd_chosen_subtypes_changes
#| fig-height: !expr pvd_line_height
#| fig-width: !expr pvd_line_width
#| fig-cap: !expr glue::glue("Change in estimated event sequence between subtypes.")
#| layout-ncol: 3
#| layout-nrow: 1
#| fig-subcap: 
