...
event_name = "symptom"
event_name_sentence_start = stringr::str_to_sentence(event_name)
```
...
### `r event_name_sentence_start` of neurodegenerative events

We analyzed measurements of 
`r length(biomarker_levels)` ordinal `r event_name`s
with a total of `r sum(nlevs)` levels, 
listed in @tbl-biomarker-list.
Herein the term "`r event_name`s" refers to 
...
summarize the `r event_name` data, 
stratified by 
...
We assumed that missing `r event_name` data 
...
the CVIC criterion [@young2018uncovering]
(summarized in Supplementary Materials @sec-cvic) 
and the OOFLL criterion [@young2018uncovering]
(summarized in Supplementary Materials @sec-oofll),
both of which measure how well models containing a given number of subtypes
