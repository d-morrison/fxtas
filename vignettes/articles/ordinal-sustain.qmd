---
title: "Development and progression of Fragile X-associated tremor/ataxia syndrome 
revealed by Subtype and Stage Inference (SuStaIn) analysis"
...
compact_fig_cap2 = paste0(
  ...
  "biomarkers that moved to earlier positions. ",
...
from `r nrow(v1_usable)` participants from the two cohorts,
consisting of 
`r nrow(v1_usable_cases)` fragile X premutation carriers and 
`r nrow(gp34_v1_controls)` controls. 
[David R Hessl, does this passage answer your question about the purpose of the controls?]{.comment-start
author="Ezra Morrison"}
The controls' data are used to estimate the distribution of the analyzed symptoms 
among non-FXTAS individuals,
and the cases' data are then used to estimate FXTAS event sequences,
...
### `r outcomes_name_sentence_start` of neurodegenerative events
...
Ordinal Subtype and Stage Inference ("SuStaIn") algorithm 
[@young2021ordinal] 
...
Ordinal SuStaIn uses Markov Chain Monte Carlo (MCMC) sampling 
...
2. CGG repeats (<100 vs ≥100) ([Supplementary @sec-methods-strat-cgg]);
3. both sex and CGG repeats (<100 vs ≥100) 
   ([Supplementary @sec-stratified-by-cgg-and-sex]).
...
The study protocols changed over the years 
(`r min_year` - `r max_year`) 
as new evaluation instruments were adopted; 
much of the missingness was due to 
adding additional measures in later stages of the studies. 
...
the Cross-Validation Information Criterion (CVIC) [@young2018uncovering]
(summarized in Supplementary Materials @sec-cvic) 
and the Out-Of-Fold Log-Likelihood (OOFLL) criterion [@young2018uncovering]
(summarized in Supplementary Materials @sec-oofll);
...
@fig-first-only shows the estimated event sequences 
stratified by sex 
and compares the estimated sequences between males and females.
...
threshold-level Mood Disorder symptoms 
prior to FXTAS Stage 1 diagnosis,
whereas male carriers developed these symptoms 
between FXTAS Stages 3 and 4 (@fig-first-only-2).
...
The out-of-fold log-likelihood (Supplementary Materials, @eq-def-oof-llik) 
...
While many psychiatric illness have a strong genetic predisposition, the lifetime prevalences here persuasively suggest that the premutation carrier state itself increases the risk of psychiatric illness. As the SCID assesses lifetime illness risk, it is probable that the index episode of psychiatric illness antedated the onset of tremor, ataxia, and major neurocognitive disorder seen in more advanced premutation conditions like FXTAS.  As such, the high lifetime burden of psychiatric illness in this population cannot be plausible attributed solely (perhaps substantially) to an “emotional reaction” to the loss of function associated with these functional impairments but should be understood as a component of the premutation carrier state itself.  It is likely that this intrinsic vulnerability to psychiatric illness is a significant component of the carrier state.  Furthermore, as only a small % of the patients had MMSE scores in the impaired range (please validate, see above comment), it is also unlikely that the high rates of depressive, anxiety (and other) psychiatric illness could be attributed to complications of FXTAS dementia, as the majority of subjects had normal MMSE scores, which would not be consistent with dementia. For example, depressive disorders are a common psychiatric complication of dementia, but few of the depressive disorders found here could be attributed to dementia.  A common sequence of illnesses seen clinically in carriers is depressive and/or anxiety disorders with adult onset, then tremor and ataxia in the 40s, followed by FXTAS dementia in the 60s.  Seen in this light, and knowing the ultimate CNS vulnerability of the carrier state, retrospectively the initial presentation of depressive and/or anxiety disorders could be regarded as a psychiatric prodrome of later full spectrum illness.  This is analogous to the psychotic illness seen in SLE before rheumatologic findings and the depressive disorder that is commonly seen in the 5 years before the motor signs leading to a diagnosis of multiple sclerosis. 
