...
scid_long_vars <- c(
  ...
  "Stimulants (SUD20)", "Opioid (SUD21)", "Cocaine (SUD22)", 
...
```{r}
scid_long |> dplyr::filter(Current == "Present", Lifetime %in% c("Absent", NA))
...
```{R}
gp34 |> 
  dplyr::filter(`FXS ID` == "104279-100") |> 
...
```{r}
scid_long |> dplyr::filter(!is.na(Current), is.na(Lifetime))
```
...
summary(
  arsenal::tableby(
    `FX**` ~ `SCID: mood disorders` + `SCID: substance use disorders` + 
      `SCID: anxiety disorders` + `SCID: somatoform disorders` + `SCID: psychotic symptoms`,
    data = scid_domains,
    test = FALSE
  )
)
