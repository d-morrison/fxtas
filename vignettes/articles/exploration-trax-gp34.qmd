...
table1(na.is.category = FALSE, 
  ...
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = males_gp34_trax_v1 |> 
    filter(`Any tremor (excluding head)` == "No tremors recorded" |
...
n_age_NoTremor <- males_gp34_trax_v1 |> 
  filter((is.na(`Any tremor (excluding head)`) | 
...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = males_gp34_trax_v1 |> filter(Study == "GP4"),
  footnote = footnote)
...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = males_gp34_trax_v1 |> filter(Study == "GP4" & `Head tremor` == "Yes"),
  footnote = footnote)
...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = males_gp34_trax_v1 |> filter(Ataxia == "Yes"))
```
...
table1(na.is.category = FALSE, 
  ...
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  footnote = footnote,
  data = males_gp34_trax_v1 |> filter(Ataxia == "No" | is.na(Ataxia))
)
```
...
) # |> sapply(F = function(x) grep(value =  TRUE, fixed = TRUE, pattern = x, x = names(males_gp34_trax_v1))) |> as.vector()
...
cantab_vars = c(
  ...
  "RTI Five-choice movement time*") 
# |> 
#   sapply(F = function(x) grep(value =  TRUE, fixed = TRUE, pattern = x, x = names(males_gp34_trax_v1))) |> 
# as.vector()
