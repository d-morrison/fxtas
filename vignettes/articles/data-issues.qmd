...
`r apoe.n |> dplyr::filter(first_missing) |> dplyr::pull(any_nonmissing) |> sum()` of these individuals have later visits with non-missing ApoE results; for example:

```{r}

gp34 |> dplyr::filter(
  # `FXS ID` == "500011-122"
  `FXS ID` == apoe.n |> dplyr::filter(first_missing, any_nonmissing) |> head(1) |> dplyr::pull(`FXS ID`)
) |> 
  dplyr::select(`FXS ID`, `Event Name`, `Visit Date`, ApoE) |> 
  pander()
...
gp34 |> 
  semi_join(apoe.n |> dplyr::filter(n_vals > 1), by = "FXS ID") |> 
  dplyr::select(`FXS ID`, `Visit Date`, `Event Name`, ApoE) |> 
...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = visit1 |> dplyr::filter(Study == "GP4"),
  footnote = footnote)
...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = visit1 |> dplyr::filter(Study == "GP4" & `Head tremor` == "Yes"),
  footnote = footnote)
...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = visit1 |> dplyr::filter(Ataxia == "Yes"))
```
...
table1(na.is.category = FALSE, 
  ...
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  footnote = footnote,
  data = visit1 |> dplyr::filter(Ataxia == "No" | is.na(Ataxia))
)
```
...
tab = gp34 |> dplyr::filter(is.na(`Visit Date`)) |> 
