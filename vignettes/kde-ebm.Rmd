...
```{r}
a = py$mcmc_samples
b = a |> sapply(X = _, F = function(x) x$ordering + 1)
...
b4 = b3 |> as_tibble() |> mutate(position = 1:nrow(b3))
b5 = b4 |>  tidyr::pivot_longer(cols = biomarkers) |> 
  mutate(name = factor(name, levels = ))
ggplot(b5, aes(x = position, y = name, fill = value)) + geom_tile()
```
