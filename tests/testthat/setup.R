...
do_manual_msoffice_snapshot_testing <- TRUE
...
skip_if_not_local_testing <- function(min_pandoc_version = "2",
                                      check_html = FALSE) {
  ...
  if (!is.null(min_pandoc_version)) {
    # Can be turned off with NULL
...
handle_manual_snapshots <- function(snapshot_folder,
                                    snapshot_name,
  ...
  browser()
  main_inst_folder <- here::here("snapshots_for_manual_tests",
                                  package = package) |>
    fs::dir_create()
  ...
  if (!file.exists(snapshot_file)) {
    stop("Following snapshot file not found in {flextable}:",
         snapshot_file)
  }
